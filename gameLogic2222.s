...
checkRight:
	...
	bne	checkLeft		//Branch if values do not match
...
checkLeft:
	...
	bne	checkUp			//Branch if values do not match
...
checkUp:
	...
	bne	checkDown		//Branch if values do not match
...
checkDown:
	...
	bne	checkStart		//Branch to pressButtonA since all directional buttons werent press
...
pressButtonA:
	...
	bne	looping			//Branch if the "A" button wasn't pressed
	bl	pressedAButton		//Branch to pressedButton
	b	looping			//Branch to looping
...
up:	
	...
	bl	fillSpaceDown		//Branch to fillSpaceDown 
...
fillSpaceUp:
	push	{lr}
	mov	r9,	#0
	str	r9,	[r1, #-1, lsl #2]//Fill space where player was when it moved down (fill space up)
	pop	{lr}
	bx	lr
fillSpaceLeft:
	push	{lr}
	mov	r9,	#0
	str	r9,	[r1, #-16, lsl #2]//Fill spacw where player was when it moved right (fill space right)
	pop	{lr}
	bx	lr
fillSpaceRight:
	push	{lr}
	mov	r9,	#0
	str	r9,	[r1, #16, lsl #2]//Fill space where player was when it moved left (fill space left)
	bx	lr
//********************************
